...
class Evaluator:
    def __init__(self, data_path=None):
        ...
        if data_path is None:
            data_path = "tasks/tsp_tour_minimisation/data"
            
        self.instances = self._load_instances(data_path)
        
        if not self.instances:
            print(f"WARNING: No data files found in {data_path}. Please download .tsp files.")
        else:
    ...
    def _load_instances(self, data_path):
        ...
        """
        shortest_tour = {"ar9152.tsp": 837479, "ca4663.tsp": 1290319, "dj38.tsp": 6656, "eg7146.tsp": 172386, "ei8246.tsp": 206171, "gr9882.tsp": 300899, "ja9847.tsp": 491924,
                         "kz9976.tsp": 1061881, "lu980.tsp": 11340, "mu1979.tsp": 86891, "nu3496.tsp": 96132, "pm8079.tsp": 114855, "qa194.tsp": 9352, "rw1621.tsp": 26051,
                         "tz6117.tsp": 394718, "wi29.tsp": 27603, "ym7663.tsp": 238314, "zi929.tsp" : 95345}
        instances = []
        for filepath in glob.glob(os.path.join(data_path, "*.tsp")):
            ...
            if dimension > 0 and len(cities) == dimension:
                ...
                if filename in shortest_tour:
                    ...
                    print(f"Skipping {filename}: No shortest tour record found in dictionary.")
        return instances
    ...
    def evaluate(self, code_string):
        ...
        with tempfile.NamedTemporaryFile(suffix=".cpp", mode="w", delete=False) as temp_cpp:
        ...
        if compile_process.returncode != 0:
            err_msg = compile_process.stderr.strip().splitlines()[-1] if compile_process.stderr else "Unknown Compilation Error"
        ...
        for idx, instance in enumerate(self.instances, 1):
            ...
            full_feedback += f" {instance['name']} has {instance['dimension']} cities.\n "
            try:
                ...
                if run_process.returncode != 0:
                    err_msg = run_process.stderr.strip().splitlines()[-1] if run_process.stderr else "Runtime Error"
                ...
                if len(tour) != N:
                    full_feedback += f"[Fail] Returned {len(tour)} cities, expected exactly {N}.\n"
                    continue
                ...
                if ratio >= 1.0:
                    ...
                    save_path = os.path.join(save_dir, f"record_breaker_{instance['name']}.cpp")
        ...
        return final_fitness, f"Perfect run! The average ratio of shortest tour to generated tour distance: {final_fitness:.4f}\n{full_feedback}"